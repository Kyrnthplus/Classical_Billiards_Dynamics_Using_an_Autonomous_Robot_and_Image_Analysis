...
Simu=0#int(input('Simular? 1(Sim) 0(Não): '))
delet=0#int(input('Deletar simulações ruins?? 1(Sim) 0(Não): '))
...
print("############################################################")
opcao = str(input("Todos os arquivos (digite '0') ou os que não forão processados (digite '1')?, caso um aarquivo especifico digite o nome: "))
...
for Nome0 in arquivos:
    try:
        ...
        # Calcula as componentes da velocidade Vx e Vy usando a derivada numérica
        Vxr = np.gradient(Xr, dt)
        Vyr = np.gradient(Yr, dt)

        # Calcula a magnitude da velocidade V
        Vr = np.sqrt(Vxr**2 + Vyr**2)
        # Calcula a média e o desvio padrão da magnitude da velocidade
        Vmr = np.mean(Vr)
        Lista_Vel.append((Vmr))
        Vrstd = np.std(Vr)
        print(f'Velocidade real:{Vr} | Vmr: {Vmr} | Vrstd: {Vrstd}')
        ...
        N_Yy=h/Altura_y #H/Altura_Y #Fator de normalização
        N = len(Cx)
        ...
        Xx = (X-np.mean(X))*N_Yy
        Yy = (Y-np.mean(Y))*N_Yy
        print("Altura teorica V: "+str(h))
        print("Altura V: "+str(np.max(Yy)+np.abs(np.min(Yy))))
        print("largura V: "+str(np.max(Xx)+np.abs(np.min(Xx))))
        ...
        for i in range(len(Xr)):
            ...
            data.write('%f %f %f %f\n' % (float(Xr[i]), float(Yr[i]), float(X[i]), float(Y[i])))
        data.close()
        ...
        Zeta=N_y
        Lista_LEG.append((Nome_sem_extensao,gamma, LE, Zeta, raio_inf,raio_sup,modulo_AB/2,))
        ...
        caminho_arquivo_Colisao = 'DadosTratados/' + str(nome_arquivo) + '_(Traj).dat'
        X, Y, x, y = np.loadtxt(caminho_arquivo_Colisao, unpack=True)
        # Load collision data
        ...
        for i in range(Tam):
            for k in range(Tam):
                if DADOS0[i][1] <= 0.1 and DADOS0[k][1] >= 0.9:
                    if abs(DADOS0[i][1] - DADOS0[k][1]-1) < e1 and abs(DADOS0[i][2] - DADOS0[k][2]-1) < e1:                 
                        MQ[i][k] = 1
                        SOMA +=1  
                if abs(DADOS0[i][1] - DADOS0[k][1]) < e1 and abs(DADOS0[i][2] - DADOS0[k][2]) < e1:                       
                    MQ[i][k] = 1
                    SOMA +=1
        RR = SOMA / (Tam * Tam)
        RR_f = RR
        ...
        plt.suptitle(f'{nome_arquivo} | 'r'$\gamma$' f' = {gamma:.3g} | 'r'$\lambda$'f' = {LE:.3g} | Colisão = {len(EffAng)} | $RR$={RR_f:.3g} | 'r'$\varepsilon$' f' = {e1}', fontsize=FS*3/4)
        ...
        #plt.plot(X, Y, color='black', linestyle='solid', drawstyle='default', linewidth=1, alpha=0.75)
        plt.scatter(X, Y, color='black', s=0.1, alpha=0.55 ,zorder=2, marker='o')
        plt.plot(X_C, Y_C, color=COR, linestyle='none', drawstyle='default', linewidth=1, marker='.', markersize=10, markeredgecolor='black',zorder=3)
        plt.xlabel(r'$X$', fontsize=FS)
        plt.ylabel(r'$Y$', fontsize=FS)
        plt.xticks([-1.5,-1, -0.5, 0, 0.5, 1,1.5], fontsize=FS*3/4)
        plt.yticks([-1.5,-1, -0.5, 0, 0.5, 1,1.5], fontsize=FS*3/4)
        #plt.axis('square')
        plt.ylim(-1.01, 1.01)
        plt.xlim(-1.01, 1.01)
        ...
        x_ticks_positions = [0, 0.5 * pi_value, pi_value, 1.5 * pi_value, 2 * pi_value]
        x_ticks_labels = ['0', r'$\frac{\pi}{2}$', r'$\pi$', r'$\frac{3\pi}{2}$', r'$2\pi$']
        plt.xticks(x_ticks_positions, x_ticks_labels, fontsize=FS*3/4 )
        plt.xlabel(r'$\ell$', fontsize=FS)
        plt.ylabel(r'sin $\phi$', fontsize=FS)
        plt.yticks([-1, -0.5, 0, 0.5, 1], fontsize=FS*3/4)
        plt.ylim(-1, 1)
        plt.xlim(-0.2, 2 * np.pi + 0.2)
        plt.minorticks_on()
        plt.minorticks_on()
        ...
        plt.subplot(1, 3, 3)
        plt.minorticks_on()
        plt.xticks(fontsize=FS*3/4)
        plt.yticks(fontsize=FS*3/4)
        plt.minorticks_on()
        plt.grid(which='minor', linestyle=':', linewidth='0.1', color='gray',alpha=0.25)
        plt.grid(which='major', linestyle='-', linewidth='0.5', color='gray', alpha=0.25)
        
        plt.plot(x_coords, y_coords, color=COR, linestyle='none', drawstyle='default', linewidth=1, marker='o', markersize=5, markeredgecolor='black',alpha=1)
        #plt.title(f'RR={RR_f} | e = {e1}')
        plt.xlabel(r'Índice $i$', fontsize=FS)
        plt.ylabel(r'Índice $j$', fontsize=FS)
        plt.tick_params(axis='both', which='both', direction='in', top=True, right=True, bottom=True, left=True)        
        plt.subplots_adjust(left=0.075, right=0.975, top=0.85, bottom=0.15, wspace=0.25)


        #plt.show()
        plt.savefig('Gráficos/' + str(nome_arquivo) + '.pdf', dpi=350)
        plt.close('all')
        ...
        plt.plot(X_C, Y_C, color=COR, linestyle='none', drawstyle='default', linewidth=1, marker='.', markersize=12.5, markeredgecolor='black',zorder=3, label='Colisão')
        plt.legend(loc='upper right', fontsize=FS*3/4)
        plt.xlabel(r'$X$', fontsize=FS)
        plt.ylabel(r'$Y$', fontsize=FS)
        plt.xticks([-1.5,-1, -0.5, 0, 0.5, 1,1.5], fontsize=FS)
        plt.yticks([-1.5,-1, -0.5, 0, 0.5, 1,1.5], fontsize=FS)
        #plt.axis('square')
        plt.ylim(-1.01, 1.01)
        plt.xlim(-1.01, 1.01)
        ...
        plt.savefig('Gráficos/' + str(nome_arquivo) + '_TC.pdf', dpi=350)
    ...
    ######################################################
        ...
        #print(len(y_traj))  # Verifica o tamanho de y_traj
            ...
            '''
        # ==========================================
        # NOVO PLOT: sen phi vs el (Gráfico Isolado Tiago)
        # ==========================================
        plt.close('all')
        
        # Recriando as proporções originais do seu outro script
        NX_T = 9
        NY_T = 6
        FS_T = 30
        PAD_T = 10
        ...
        # Plota os dados de Poincaré que já foram carregados em X_P e Y_P
        plt.plot(X_P, Y_P, color=COR, linestyle='none', drawstyle='default', 
                 linewidth=1, marker='.', markersize=10, markeredgecolor='black', label='Simulação')
        
        # Configurações de Eixos e Fontes iguais ao AE e An
        ...
        # Ajuste de layout para as proporções de 9x6
        plt.subplots_adjust(left=0.15, right=0.95, top=0.90, bottom=0.20)
        
        # Salva com o nome do arquivo + Tiago.pdf
        plt.savefig('Gráficos/' + str(nome_arquivo) + 'Tiago.pdf')
        plt.close('all')
        # ==========================================
    ######################################################
...
with open('Lyapunov/N_G_LE_Ri_Rs_M_valores.txt', 'w') as arquivo_txt:
    arquivo_txt.write('Nome do Exp | Gamma | Expoent Lyapunov | Zeta | | R_i | R_s | Modulo\n')
    for item in Lista_LEG:
        arquivo_txt.write(f'{item[0]} {item[1]} {item[2]} {item[3]} {item[4]} {item[5]} {item[6]} \n')
with open('Lyapunov/Velo_valores.txt', 'w') as arquivo_txt:
    arquivo_txt.write('Velocidade\n')
    for i in range(len(Lista_Vel)):  # Corrigir a iteração sobre o comprimento da lista
        arquivo_txt.write(f'{Lista_Vel[i]}\n')  # Escrever o valor correto da lista
