...
Xsf_P, Ysf_P, Ysfe_P = np.loadtxt(caminho_arquivo_Poincare, unpack=True)
#X_P = X_P * 2 * np.pi / np.max(X_P)

caminho_arquivo_Poincare = 'DadosCrus/LG/' + str(nome_arquivo3)
Xf_P, Yf_P, Yfe_P = np.loadtxt(caminho_arquivo_Poincare, unpack=True)
#X_P = X_P * 2 * np.pi / np.max(X_P)
...
plt.errorbar(X_P, Y_P, xerr=Xe_P, yerr=Ye_P, color=COR, linestyle='none', drawstyle='default', linewidth=1, marker='.', markersize=18, markeredgecolor='black', capsize=4, elinewidth=2, label='Experimental', zorder=2)

plt.errorbar(Xsf_PT, Ysf_PT, linestyle='--', yerr=Ysfe_PT, color='blue', ecolor="black", drawstyle='default', linewidth=1, marker='.', markersize=9, markeredgecolor='black', alpha=1, capsize=3, elinewidth=1, label='Numerical', zorder=1)
plt.errorbar(Xf_PT, Yf_PT, linestyle='--', yerr=Yfe_PT, color='green', ecolor="black", drawstyle='default', linewidth=1, marker='.', markersize=9, markeredgecolor='black', alpha=1, capsize=3, elinewidth=1, label='Numerical', zorder=1)
# Legenda
...
plt.savefig('Gráficos/LyapGamma/' + str(nome_arquivo) + '.png', dpi=600)
