...
def processar_arquivo(nome_arquivo, Quant):
...
Quant = int(input('Digite a quantidade de passos, (0 para todos): '))
inicio = time.time()
arquivos = sorted([arq for arq in os.listdir('DadosCrus') if arq.endswith('.txt')])

# Número de processos a serem utilizados
num_processos = os.cpu_count()

with Pool(num_processos) as p:
    p.starmap(processar_arquivo, [(arq, Quant) for arq in arquivos])
