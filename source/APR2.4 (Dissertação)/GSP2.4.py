...
for Nome in arquivos:
    ...
    plt.plot(X, Y, color=COR, linestyle='none', drawstyle='default', linewidth=1,marker='.', markersize=0.5, label='Trajetória ')
    plt.plot(X_C, Y_C, color=COR, linestyle='none', drawstyle='default', linewidth=22, marker='.', markersize=7.5, markeredgecolor='black', label='Colisão')
    ...
    plt.xticks(np.arange(-1, 1.01, 0.5),fontsize=FS*4/5)
    #plt.xticks(np.linspace(-1.5,1.5,5),fontsize=FS*4/5)
    plt.ylim(-1.05, 1.05)
    plt.xlim(-1.05, 1.05) #-(1.55+1.55*0.05)*N, +(1.55+1.55*0.05)*N)
    ...
    #plt.tight_layout()
    plt.subplots_adjust(left=0.25, right=0.8, top=0.95, bottom=0.15)

    plt.grid(True)

    plt.savefig('Gráficos/SP/' + str(nome_arquivo) + '.SP.pdf')
