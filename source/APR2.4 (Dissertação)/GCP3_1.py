...
for Nome in arquivos:
    ...
    #plt.rcParams["font.serif"] = "/usr/share/fonts/opentype/freefont/FreeSerif.otf"

    plt.plot(X_PT, Y_PT, color='black', linestyle='none', drawstyle='default', linewidth=1, marker='.', markersize=1,alpha=1)#, markeredgecolor='black',alpha=0.5)
    ...
    plt.xlabel(r'$\ell$', fontsize=FS)#, labelpad=PAD)
    plt.ylabel(r'sin $\phi$', fontsize=FS)#, labelpad=PAD)
    plt.xticks([0, 0.5 * pi_value, pi_value, 1.5 * pi_value, 2 * pi_value], ['0', r'$\frac{\pi}{2}$', r'$\pi$', r'$\frac{3\pi}{2}$', r'$2\pi$'], fontsize=FS)
    ...
    plt.savefig('Gráficos/Poincaré/Comp/png/' + str(nome_arquivo) + '_(Comp_Poincare_Comp).png', dpi=600)
    plt.savefig('Gráficos/Poincaré/Comp/pdf/' + str(nome_arquivo) + '_(Comp_Poincare_Comp).pdf', dpi=600)
    plt.savefig('Gráficos/Poincaré/Comp/svg/' + str(nome_arquivo) + '_(Comp_Poincare_Comp).svg', dpi=600)
    #plt.show()
